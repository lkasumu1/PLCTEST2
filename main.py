...
#since / can be used for division or comments this method processes both
def div_or_cmt(err_line, err_col):
  if next_ch() != '*':
    return tk_Div, err_line, err_col

  # this method look for comments
  next_ch()
  while True:
    if the_ch == '*':
      if next_ch() == '/':
...
# this method checks for a string
...
#*** method checks for identifiers and integers
def ident_or_int(err_line, err_col):
  ...
  if len(text) == 0:
    error(
      err_line, err_col, "ident_or_int: unrecognized character: (%d) '%c'" %
      (ord(the_ch), the_ch))
...
# checks for comparision operators
def follow(expect, ifyes, ifno, err_line, err_col):
  ...
  if ifno == tk_EOI:
    error(err_line, err_col,
          "follow: unrecognized character: (%d) '%c'" % (ord(the_ch), the_ch))
...
def gettok():
  ...
  err_line = the_line
  err_col = the_col
  # checks for these char literals
